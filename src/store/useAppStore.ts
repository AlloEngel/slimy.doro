...
interface AppState {
  ...
  secondsLeft: number;
  isRunning: boolean;
  cycle: number; // 1-indexed focus cycle within the long-break loop
...
export const useAppStore = create<AppState>((set) => ({
  ...
  toggleShowTodo: () =>
    set((state) => {
      ...
      persistSettings(settings);
      return { settings, todoExpanded: settings.showTodo ? state.todoExpanded : false };
    }),
  ...
  updateTimerSettings: (partial) =>
    set((state) => {
      ...
      persistSettings(settings);
      // If idle (not running), reflect new durations immediately.
  ...
  toggleTodoExpanded: () => set((state) => ({ todoExpanded: !state.todoExpanded })),
  ...
  tick: () =>
    set((state) => {
      ...
      // Session complete — advance mode, fire celebration + notification.
